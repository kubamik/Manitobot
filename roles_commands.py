...
class PoleceniaPostaci(commands.Cog, name="Polecenia postaci i frakcji",
    ...
    async def arrest(self, ctx, *, member):
      ...
      try:
        if bot.game.active_player.player.member != player:
          raise AttributeError
        await bot.game.active_player.activity(ctx, member)
    ...
    async def drink(self, ctx, *, member):
      ...
      player = get_member(ctx.author.id)
      try:
        if bot.game.active_player == None or  bot.game.active_player.player.member != player:
          raise AttributeError
        await bot.game.active_player.activity(ctx, member)
    ...
    async def sedzia(self,ctx):
      ...
      if czy_manitou(ctx):
        try:
          me = bot.game.role_map["Pijany_Sędzia"]
        except:
          try:
            me = bot.game.role_map["Sędzia"]
          except:
      ...
      if nickname[-1] != ')':
        try:
          await member.edit(nick=nickname + "({})".format(bot.game.player_map[member].role))
        except discord.errors.Forbidden:
          await ctx.send("Nie mam uprawnień aby zmienić nick")
        await get_town_channel().send("Rola **{}** to **{}**".format(nickname.replace('+',' '),bot.game.player_map[member].role))
      else:
    ...
    async def burmistrz(self,ctx):
      ...
      if czy_manitou(ctx):
        try:
          me = bot.game.role_map["Burmistrz"]
        except:
    ...
    async def refuse(self, ctx):
      ...
      try:
        player = bot.game.player_map[member]
