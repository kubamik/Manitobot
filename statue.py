...
from utility import playing, GameEnd, send_to_manitou, get_town_channel, \
    get_manitou_notebook
...
class Statue:
    ...
    async def if_followed(self, member):
        for auth, mem in self.followed.items():
            if mem == member and member != self.holder:
                ...
                c = "{} przejmuje posążek w wyniku śledzenia".format(
                    globals.current_game.player_map[auth].role)
    ...
    def day_search(self, member):
        if self.holder == member:
            raise GameEnd("Znaleziono posążek u {}".format(member.display_name),
                          "Miasto")
        else:
            return "U **{}** posążka nie znaleziono".format(member.display_name)
    ...
    async def present(self, member, role):
        ...
        if not f:
            ...
            globals.current_game.nights[
                -1].output = "{} przejmuje(-ą) posążek".format(faction)
        else:
            globals.current_game.nights[-1].output = f
