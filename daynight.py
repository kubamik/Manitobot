...
night_order = ["Lucky_Luke", "Dziwka", "Szeryf", "Pastor", "Opój",
               "Pijany_Sędzia", "Hazardzista", "Lusterko", "Bandyci",
               "Mściciel", "Złodziej", "Szuler", "Szaman", "Indianie",
               "Lornecie_Oko", "Ufoki", "Inkwizycja"]
...
class Night:
    ...
    def what_next(self):
        try:
            for i in night_order[self.active_number:]:
                if (i in globals.current_game.role_map and
                    globals.current_game.role_map[
                        i].work()) or i in globals.current_game.faction_map:
                    return ' ➡️{}'.format(i)
            return ' ➡️koniec nocy'
        except IndexError:
            return ''
    ...
    async def night_next(self, channel=None):
        ...
        if self.active_faction is None:
            try:
                ...
                await channel.send(
                    "Nie ma więcej postaci. Zakończ noc komendą `&day`")
                self.active_faction = None
                self.active_role = None
                return
        if not self.active_faction is None:
            ...
            try:
                ...
                self.last_mess = await channel.send(
                    "Rozpoczynamy turę {}.".format(next.replace('_', ' ')) + c)
                await self.last_mess.add_reaction('➡️')
            except InvalidRequest as err:
                if err.reason is None:
            ...
            c = self.what_next()
            try:
                ret = await globals.current_game.role_map[
                    next].new_night_start()
                await globals.current_game.role_map[next].player.member.send(
                    ret)
                self.last_mess = await channel.send(
                    "Wysłano instrukcje do {}".format(
                        next.replace('_', ' ') + c))
                await self.last_mess.add_reaction('➡️')
            except InvalidRequest as err:
                if err.reason is None:
    ...
    async def bishop(self, role_class, member):
        c = "Biskup przerwał grę, aby zabić {}, konieczne jest ustalenie posiadacza posążka".format(
            member)
