...
class Activity:
    ...
    def angel_alive(self):
        ...
        try:
            if globals.current_game.role_map[
                r].player.member in get_dead_role().members:
                raise InvalidRequest()
        except KeyError:
            raise InvalidRequest()
            ...
            if p.player.member not in get_dead_role().members:
                ...
                if o != None:
                    await self.player.member.send(
                        "Pastor sprawdził {}, członka frakcji {}".format(
                            o.member.display_name, get_faction(o.role)))
                else:
                    await self.player.member.send(
                        "Pastor nie sprawdzał tej nocy")
                raise InvalidRequest(
                    "{} otrzymał informacje o Pastorze".format(self.name))
        except KeyError:
            pass
    ...
    async def statue_none(self):
        if globals.current_game.statue.holder is None:
            await globals.current_game.nights[-1].bishop(self,
                                                         self.member.member.display_name)
            self.my_activities["burn"] -= 1
            raise InvalidRequest(
                "Zabójstwo zostanie zrealizowane po ustaleniu posiadacza posążka")
    ...
    def if_member(self):
        if self.member is None:
            raise InvalidRequest(
                "Musisz działać według kolejności opisanej w instrukcji")

    def check_heresis(self):
        fac = give_faction(self.member.role)
        if fac in ["Indianie", "Ufoki", "Janosik", "Lusterko", "Murzyni",
                   "Bogowie"]:
    ...
    async def deactivate_cond(self):
        if self.count == -2 and not "copied" in self.my_activities:
            ...
            c = "Poczekaj na decyzję o dodatkowej zdolności {}".format(
                self.name)
    ...
    def unchange(self):
        if self.count == -2:
            self.my_activities = permissions.role_activities[self.name][
                2].copy()
            self.my_activities["copy"] = 0

    async def search_send(self):
        c = "{} przeszukał(-a) {}".format(self.name,
                                          self.member.member.display_name)
        await send_to_manitou(c)
        await get_manitou_notebook().send(c)
    ...
    async def angelize(self):
        ...
        await angel.send(
            "Skorzystałeś ze swojej umiejętności twoja nowa rola to: {}".format(
                self.name))
        c = "{} jako Anioł przejął rolę {}, czyli {}".format(angel.display_name,
    ...
    def if_holders(self):
        if give_faction(
                self.member.role) == globals.current_game.statue.faction_holder:
        ...
        try:
            if globals.current_game.days[-1].hang_final:
                await get_town_channel().send(
                    "Burmistrz ułaskawił wieszaną osobę")
                await get_glosowania_channel().send(
                    "Burmistrz ułaskawił wieszaną osobę")
    ...
    async def reveal(self):
        ...
        await get_town_channel().send(
            "Rola **{}** to **{}**".format(nickname.replace('+', ''),
                                           self.name.replace('_', ' ')))
        try:
            await member.edit(
                nick=nickname + "({})".format(self.name.replace('_', ' ')))
        except discord.errors.Forbidden:
            await member.send(
                "Zmień swój nick na {}, bo ja nie mam uprawnień.".format(
                    nickname + "({})".format(self.name.replace('_', ' '))))
    ...
    def if_duel(self):
        if not self.member is None:
            ...
            if not self.member.member in globals.current_game.days[
                -1].participants:
    ...
    async def change_duel(self):
        if not self.member is None and not "copied" in self.my_activities:
            await globals.current_game.days[-1].change_winner(
                self.member.member)
    ...
    async def search(self):
        found = await globals.current_game.statue.search(self.player.member,
                                                         self.name,
                                                         self.member.member,
                                                         not self.if_has())
        return found
    ...
    def refusal(self):
        if "refused" in self.my_activities:
            globals.current_game.nights[-1].output += self.my_activities[
                "refused"]

    def can_hold(self):
        if self.member.role not in self.roles:
            raise InvalidRequest(
                "Możesz wręczyć posążek tylko członkowi swojej frakcji")
    ...
    async def special_search(self):
        found = await globals.current_game.statue.special_search(
            self.player.member, self.name, self.member.member,
            not self.if_has())
        return found
    ...
    async def present(self):
        await globals.current_game.statue.present(self.member.member,
                                                  self.member.role)
    ...
    async def f_search(self):
        return await globals.current_game.statue.faction_search(self.name,
                                                                self.member.member,
                                                                not self.f_has())
    ...
    def if_not_prev(self):
        if self.member == self.roled_members[-1]:
            raise InvalidRequest(
                "Nie możesz użyć swojej zdolności na tej samej osobie dwa razy z rzędu")

    def check_faction(self):
        return "Frakcja {} to {}".format(self.member.member.display_name,
                                         get_faction(self.member.role))

    def check_role(self):
        return "Rola **{}** to **{}**".format(self.member.member.display_name,
                                              self.member.role.replace('_',
                                                                       ' '))

    async def hang_win(self):
        if self.die_reason == 'hang':
            reason = "Powieszony został {}, czyli {}".format(
                self.player.member.display_name, self.name)
    ...
    def got_today(self):
        try:
            if not (
                    self.faction.f_has() or globals.current_game.statue.last_change == globals.current_game.day):
                raise InvalidRequest()
        except AttributeError:
            if not (
                    self.has() or globals.current_game.statue.last_change == globals.current_game.day):
                raise InvalidRequest()
    ...
    def follow(self):
        globals.current_game.statue.follow(self.player.member,
                                           self.member.member)

    async def mirror_send(self):
        c = "{} zlustrowało {}".format(self.name,
                                       self.member.member.display_name)
        await send_to_manitou(c)
        await get_manitou_notebook().send(c)
    ...
    def detect(self):
        ...
        if me > holder > member or me < holder < member:
            return "Posążek jest na wewnętrznym łuku między Tobą i {}".format(
                member)
        return "Posążek jest na zewnętrznym łuku między Tobą i {}".format(
            member)
