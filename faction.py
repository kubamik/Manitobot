...
class Faction(Activity):
    ...
    async def night_start(self):
        ...
        for role in self.roles.values():
            if not role.player.sleeped and role.player.member not in get_dead_role().members:
                ...
                if self.leader is None:
                    ...
                    try:
                        await self.channel.set_permissions(role.player.member,
                                                           read_messages=True,
                                                           send_messages=True)
                    except discord.errors.Forbidden:
                        pass
                await role.player.member.send(
                    "Rozpoczynamy rundę twojej frakcji. Wejdź na <#{}>".format(
                        self.channel.id))
            elif role.player.sleeped or (
                    role.player.member in get_dead_role().members and not role.revealed):
                awake = False
        if not counter and not awake:
            ...
            raise InvalidRequest(
                "{} nie budzą się, bo nie mogą lub są martwi, ale nikt o tym nie wie".format(
                    self.name))
        elif not counter:
            raise InvalidRequest()
    ...
    async def faction_next(self):
        if self.operation == "hold" and globals.current_game.statue.holder == None:
            raise InvalidRequest(
                "Najpierw potrzebne jest ustalenie posiadacza posążka (dla Manitou `&give NICK`)")
        try:
        ...
        try:
            ...
            if statue == None:
                statue = self.f_has()
            if (
                    self.operation == "hold" or globals.current_game.day > 0) and statue == self.f_has():
    ...
    async def new_activity(self, ctx, operation,
        ...
        c = f_coms_manit_end[operation].format(self.name,
