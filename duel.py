...
class Duel:
    ...
    def add_dare(self, author, member):
        if self.duels_today == globals.current_game.duels:
        ...
        try:
            if author in self.dared[member] or member in self.dared[author]:
                raise InvalidRequest(
                    "Wyzwałeś już tego gracza lub on wyzwał ciebie")
        except KeyError:
            pass
    ...
    def remove_dare(self, author):
        ...
        return "**{}** odrzucił pojedynek od **{}**".format(author.display_name,
                                                            member.display_name)

    def remove_member(self, member):
        if member in self.daring:
            for player in self.daring[member]:
                if self.dared[player].index(
                        member) == 0 and player in self.duels_queue:
    ...
    def accept(self, author):
        ...
        if author in self.duels_queue:
            raise InvalidRequest(
                "Masz już oczekujący pojedynek. Następny ci nie ucieknie")
        index = 0
        ...
        return "**{}** przyjął pojedynek od **{}**".format(author.display_name,
                                                           self.dared[author][
                                                               0].display_name)
    ...
    async def if_start(self, author, member):
        ...
        try:
            if globals.current_game.role_map["Szeryf"].alive():
                c += "<@{}> czy przyjmujesz? Użyj `&przyjmuję` lub `&odrzucam`".format(
                    member.id)
    ...
    async def if_next(self, write=False):
        if len(self.duels_queue) > 0 and self.duels_order[0] == \
                self.duels_queue[
                    0] and self.duels_today < globals.current_game.duels:
            member = self.duels_queue[0]
    ...
    def dares_print(self):
        ...
        for player in self.duels_order:
            mess += "**{}** vs. **{}**".format(c[player][0].display_name,
                                               player.display_name)
        ...
        if len(self.duels_order) == 0:
            mess = 'Nie ma wyzwań'
        mess += "\nPozostało {} pojedynków".format(
            globals.current_game.duels - self.duels_today)
        return mess

    async def start_duel(self, agresor, victim):
        self.remove_duel()
        self.duelers = (globals.current_game.player_map[agresor].role_class,
                        globals.current_game.player_map[victim].role_class)
        self.participants = (agresor, victim)
        # print(self.participants)
        self.duel = True
        c = "Rozpoczynamy pojedynek:\n :gun:**{}** vs. :shield:**{}**".format(
            agresor.display_name, victim.display_name)
        await get_town_channel().send(c)
    ...
    async def result_duel(self, ctx, votes: tuple):
        results = []

        async def rew(member):
            try:
                await globals.current_game.player_map[
                    member].role_class.new_activity(ctx, "revoling")
        ...
        if results[0][1] != results[1][1]:
            self.winner = [
                globals.current_game.player_map[results[0][0]].role_class]
            c = "Pojedynek ma wygrać **{}**. Zginąć ma **{}**".format(
                results[0][0].display_name, results[1][0].display_name)
    ...
    async def end_duel(self, ctx):
        if not self.duels_result:
            raise InvalidRequest(
                "Najpierw musisz przeprowadzić głosowanie na zwycięzcę")
        loser_role = get_duel_loser_role()
        winner_role = get_duel_winner_role()
        if len(set(loser_role.members + winner_role.members)) != len(
                loser_role.members + winner_role.members):
            raise InvalidRequest(
                "Ktoś ma rozdwojenie jaźni - jest zwycięzcą i przegranym. Manitou musi to naprawić.")
        for member in get_duel_loser_role().members:
            if member not in self.participants:
                raise InvalidRequest(
                    "{} jakimś cudem przegrał pojedynek nie grając".format(
                        member.display_name))
        for member in get_duel_winner_role().members:
            if member not in self.participants:
                raise InvalidRequest(
                    "{} jakimś cudem wygrał pojedynek nie grając".format(
                        member.display_name))
        self.winner = []
        ...
        if len(self.winner) == 1:
            c = "Pojedynek wygrywa **{}**".format(
                self.winner[0].player.member.display_name)
    ...
    async def change_winner(self, member):
        for player in self.participants:
            await player.remove_roles(get_duel_winner_role(),
                                      get_duel_loser_role())
        for player in self.participants:
            if player == member:
