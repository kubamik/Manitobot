...
class Duel:
  ...
  async def if_next(self, write = False):
    if len(self.duels_queue) > 0 and self.duels_order[0] == self.duels_queue[0] and self.duels_today < globals.current_game.duels:
      member = self.duels_queue[0]
  ...
  def dares_print(self):
    ...
    if len(self.duels_order) == 0:
      mess = 'Nie ma wyzwań'
    mess += "\nPozostało {} pojedynków".format(globals.current_game.duels - self.duels_today)
    return mess

  async def start_duel(self, agresor, victim):
    self.remove_duel()
    self.duelers = (globals.current_game.player_map[agresor].role_class, globals.current_game.player_map[victim].role_class)
  ...
  async def result_duel(self, ctx, votes: tuple):
    ...
    if results[0][1] != results[1][1]:
      self.winner = [globals.current_game.player_map[results[0][0]].role_class]
      c = "Pojedynek ma wygrać **{}**. Zginąć ma **{}**".format(results[0][0].display_name,results[1][0].display_name)
