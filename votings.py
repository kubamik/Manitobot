...
Liczba głosujących: {}\n""".format(summary_readable, votes_count,
                                   votes_count // globals.current_game.required_votes)
    ...
    if end_vote:
        ...
        try:
            if globals.current_game.days[-1].duel:
                await globals.current_game.days[-1].result_duel(ctx,
                                                                voing_summary.items())
            elif globals.current_game.days[-1].hang:
                await globals.current_game.days[-1].hang_sumarize(ctx,
                                                                  voing_summary.items())
            ...
            if globals.current_game.days[-1].search:
                await globals.current_game.days[-1].search_summary(ctx,
                                                                   voing_summary.items())
    else:
        not_voted = list(set(get_player_role().members) - set(list(
            get_dead_role().members)) - globals.current_game.players_voted - set(
            globals.current_game.not_voting))
        if len(not_voted) == 0:
