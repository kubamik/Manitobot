...
class Pojedynki(commands.Cog):
    ...
    async def cog_check(self, ctx):
        try:
            return not globals.current_game.night and (
                        not globals.current_game.days[-1].duel or get_member(
                    ctx.author.id) in get_manitou_role().members)
        except:
            return False
    ...
    async def cog_command_error(self, ctx, error):
        if isinstance(error, commands.CheckFailure):
            await ctx.send(
                "Tej komendy można używać tylko w dzień i (prawdopodobnie) nie w trakcie pojedynku")
    ...
    async def duel_dare(self, ctx, *, gracz):
        ...
        try:
            ...
            if member == author:
                await ctx.send(
                    "Celujesz sam w siebie, ale przypominasz sobie, że Twój pies będzie smutny")
                return
